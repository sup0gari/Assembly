...
## PEフォーマット
Windowsにおける`.exe`や`.dll`の共通フォーマットで、プログラム実行時にどこにコードがあり、どこに関数リストがあるかを知るための情報。ファイルやメモリの先頭に必ず書き込まれている。
- `DOS Header`  
すべてのWindowsファイルの先頭にある互換性維持用のヘッダー。  
`e_lfanew`という関数にNT Headersへのオフセットが記載されている。
- `NT Headers`  
DLLのビット数や、関数のアドレスなどが記されているヘッダー。
`DataDirectory`という配列の0番目にExport Directoryという関数のアドレスリストへのベースアドレスからのRVA（オフセット）が記載されている。
- `Sections`  
`.text`や`.data`などの領域
